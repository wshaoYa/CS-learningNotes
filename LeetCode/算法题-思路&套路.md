...
// 时间nlogn，空间logn
func main() {
	nums := []int{2, 3, 6, 7, 8, 5, 6, 7, 8, 9, 3, 2, 4, 5, 3, 4}
	...
	dfs(0, len(nums)-1)
	fmt.Println(nums)  // [2 2 3 3 3 4 4 5 5 6 6 7 7 8 8 9]
}
```
...
不像其他语言 （c/c++，Java，Python，Javascript，等）， Go 没有专门的一元取反位运算符。取而代之的是，XOR 运算符 ^，也可作为一元取反运算符作用于一个数字。
...
![img](https://img2018.cnblogs.com/blog/874963/201909/874963-20190930170044824-1061422457.png)

换成二进制就是

![img](https://img2018.cnblogs.com/blog/874963/201909/874963-20190930174515759-289489948.png)
...
![img](https://img2018.cnblogs.com/blog/874963/201910/874963-20191013123453790-20308625.png)
...
**注意**：以下模板在最坏的情况下（数据几乎已排序好，或数据值完全一样），时间好像仍为n²
...
// 使用快速选择算法找到数组 arr 中第 k 大的元素 （ k的范围为[0,n) )
// 处理后的arr为降序
...
```go
// quickSelectRandomized 使用随机化快速选择算法找到数组 nums 中第 k 小的元素（ k的范围为[0,n) )
// 处理后的nums为升序
func quickSelect(nums []int, k int) int {
	low, high := 0, len(nums)-1
	for low <= high {
		p := partitionRandomized(nums, low, high)
		if p == k {
			return nums[p]
		} else if p > k {
			high = p - 1
	...
	// 上述循环结束意味着找到了第 k 小的元素
	return nums[k]
}

// partitionRandomized 函数采用随机化方式选取枢轴并进行分区
// partitionRandomized 函数将数组 nums 在 pivot 位置划分，并返回 pivot 的最终位置，
// 使得 nums[:p] 的所有元素都不大于 pivot，nums[p+1:] 的所有元素都不小于 pivot。
...
[分享丨模运算的世界：当加减乘除遇上取模 - 力扣（LeetCode）](https://leetcode.cn/circle/discuss/mDfnkW/)
...
例如 此时是|A∪B∪C∪D∪E| 需枚举所有非空子集，奇正偶负（个数为奇数时前符号为正，否则为负）

以|A∪B∪C|举例：

![image-20240415172615468](https://s2.loli.net/2024/04/15/m7hcf9FC8zTX5Vr.png)

一般可借助 位运算 利用 枚举集合 的方式来帮助计算容斥原理结果？
...
num := new(big.Int).Binomial(int64(m), int64(n)).Int64()
```
...
var (
	primes   []int    // 质数
	nonPrime = [mx]bool{true, true} // 非质数
)
...
// 通过只计算nonPrime 牺牲不计算primes 从而优化时间为n
const mx = 1e3 + 1 // 范围
var (
	nonPrime = [mx]bool{true, true} // 非质数
)
...
func init() {
	for num := 2; num*num < mx; num++ { // num <mx 优化为num*num<mx ，时间降为n
		if !nonPrime[num] {
			for i := num * num; i < mx; i += num {
				nonPrime[i] = true
			}
...
func solve(n int, queries [][]int) []int {
    diff := make([]int, n) // 差分数组, 也可申请n+1空间，避免后续越界if逻辑判断
...
详情可看灵神题解讲解 [3102. 最小化曼哈顿距离 - 力扣（LeetCode）](https://leetcode.cn/problems/minimize-manhattan-distances/solutions/2716755/tu-jie-man-ha-dun-ju-chi-heng-deng-shi-b-op84/)

<img src="https://pic.leetcode.cn/1711856129-vydCEa-w391-c.png" alt="w391-c.png" style="zoom: 25%;" />

![image-20240401223921950](https://s2.loli.net/2024/04/01/qOy7hjvHCQKsego.png)

## **四平方和定理**

[四平方和定理_百度百科 (baidu.com)](https://baike.baidu.com/item/四平方和定理)
...
![image-20240526151549596](https://s2.loli.net/2024/05/26/b4osArjLFQVKYhz.png)

![image-20240526151537894](https://s2.loli.net/2024/05/26/crkTnI59KONm218.png)
...
for i := 0; i < n; {
    ...
    // 此时从 start 到 i-1 是一组，[start,i-1]闭区间
    // 下一组从 i 开始，无需 i += 1
    ans = xxx
}
```
...
![image-20240911101840879](https://s2.loli.net/2024/09/11/iC487lRqfXveW2M.png)

![image-20240911101858046](https://s2.loli.net/2024/09/11/45XWsV8I96jyLnZ.png)
