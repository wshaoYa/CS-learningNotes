...
**注意**：win 下没有 `syscall.SIGUSR1` `syscall.SIGUSR2` `syscall.SIGTSTP` 这几个常量的，而用到这几个的时候，在win 下运行就直接报错了。为了能正常运行，可以修改 `Go\src\syscall\types_windows.go` 文件，反正这几个常量在win下也是没有用的

[windows下使用endless报错：undefined: syscall.SIGUSR1 | Go 技术论坛 (learnku.com)](https://learnku.com/articles/51696)
