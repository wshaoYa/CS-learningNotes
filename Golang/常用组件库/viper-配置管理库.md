...
- 相对路径:  当前运行【可执行文件】的路径（不一定是可执行文件所在的目录路径）
- 绝对路径:  系统中实际的文件路径
...
指定配置文件名和配置文件的位置，viper自行查找可用的配置文件

- 配置文件名不需要带后缀
- 配置文件位置可配置多个

**特别注意**：viper的`AddConfigPath`方法适用场景为在使用远程服务配置（如etcd、consul等）或字节流数据 时来显式的指定配置文件的类型在本地有具体config文件时无效，写了等于没写）。所以本地有具体config文件时只需要写`SetConfigName`即可，或者显示的指定文件位置和名称`SetConfigFile`

`AddConfigPath`基本上是配合远程配置中心使用（读取字节流数据情况）的，告诉viper当前的数据使用什么格式去解析

```go
viper.SetConfigName("config") // 配置文件名称(无扩展名)
viper.SetConfigType("yaml") // 使用远程服务配置（如etcd、consul等）或字节流数据时来特别的指定配置文件的类型
...
**解答：**会先从json加载（应该是因为json的字典序小于yaml，viper先找到哪个用哪个，这里文件列表字典序排序后先找到的是json)



在上面两个语句下搭配使用`viper.SetConfigType("yaml")`指定配置文件类型可以实现预期的效果吗？

**解答：**不能，viper的`AddConfigPath`方法适用场景为在使用远程服务配置（如etcd、consul等）或字节流数据 时来显式的指定配置文件的类型，在本地有具体config文件时无效，写了等于没写 :）

所以本地有具体config文件时只需要写`SetConfigName`即可，或者显示的指定文件位置和名称`SetConfigFile`
...
**特别声明**：两种方式各有优缺点，根据所需选择合适使用即可，没有谁绝对好一说
