...
当你在本地使用 Docker 构建镜像时，构建成功后镜像文件并不会直接保存在你的项目目录中，而是存储在 Docker 守护进程的镜像存储位置。在 Windows 和 macOS 上，Docker 客户端可能与 Docker 守护进程运行在虚拟机中，因此实际镜像存储的位置可能在虚拟机中的文件系统。
...
启动时会先默认占用3306和33060（33060为mysql8以后的特殊拓展端口），而后再转移至指定的容器外部映射端口（此处为33061）

所以在启动mysql容器时，本地3306和33060均不能被占用，等待mysql容器启动完毕后，只剩下了33061被占用。此时若想同时使用本地的mysql，可再启动，占用3306和33060端口。
